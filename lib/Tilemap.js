...
Tilemap.prototype.writeXml = function (path, tilesetPath, format, cb) {
    ...
    switch(format) {
        ...
        case 'base64':
        default:
            this._doXmlWrite(path, tilesetPath, buff, null, cb);
            break;
    }
};
...
Tilemap.prototype._doXmlWrite = function(path, tilesetPath, buff, comp, cb) {
    var xml =
        ...
        '       <image source="' + tilesetPath + '" width="' + this.gridWidth * this.tileset.tileWidth + '" height="' + this.gridHeight * this.tileset.tileHeight + '"/>\n' +
        '   </tileset>\n' +
...
Tilemap.prototype._createTilemap = function () {
    ...
    this.buffer = new Buffer(4 * this.png.width * this.png.height);
    ...
    for (var y = 0; y < this.gridHeight; ++y) {
        for (var x = 0; x < this.gridWidth; ++x) {
            ...
            if (this.tileset.tileIdMap[key] !== undefined) {
                tile = this.tileset.tileIdMap[key];
                this.buffer.writeUInt32LE(tile + 1, idx);
